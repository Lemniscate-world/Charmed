...
- Update `displayInfoOfSettingName` to include `apiKey` for `ollama`.
...
- In `newOpenAICompatibleSDK`, use the API Key from settings if provided for the `ollama` provider, falling back to `'noop'` if empty.
- Update `newOllamaSDK` to accept `apiKey` and include it in the `headers` as `Authorization: Bearer <key>`.
- Update `ollamaList` and `sendOllamaFIM` to retrieve and pass the `apiKey` to `newOllamaSDK`.
